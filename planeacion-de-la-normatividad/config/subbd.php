...
function insertar($conexion,$codigo,$versio,$nombre,$categoria,$fecha,$tipo,$fecha_apro,$fecha_imp,$ubicacion,$conservacion,$Id_destino,$tipoarchivo,$archivoBLOB){
    $sql="INSERT INTO subplaneacion_de_la_normatividad(codigo,versio,nombre,categoria,fecha,tipo,fecha_apro,fecha_imp,ubicacion,conservacion,Id_destino,tipoarchivo,archivo) VALUES('$codigo','$versio','$nombre','$categoria','$fecha','$tipo','$fecha_apro','$fecha_imp','$ubicacion','$conservacion','$Id_destino','$tipoarchivo','$archivoBLOB')";
    $query=mysqli_query($conexion,$sql);
    return $query;
}
function eliminar($conexion,$id){
    $sql="DELETE from subplaneacion_de_la_normatividad WHERE id=$id";
    $query=mysqli_query($conexion,$sql);
    return $query;
}
...
function editarNombre($conexion,$id,$nombre){
    $sql="UPDATE subplaneacion_de_la_normatividad SET nombre='$nombre' WHERE id=$id";
    $query=mysqli_query($conexion,$sql);
    return $query;
}
function editarCodigo($conexion,$id,$codigo){
    $sql="UPDATE subplaneacion_de_la_normatividad SET codigo='$codigo' WHERE id=$id";
    $query=mysqli_query($conexion,$sql);
    return $query;
}
function editarVersion($conexion,$id,$versio){
    $sql="UPDATE subplaneacion_de_la_normatividad SET versio='$versio' WHERE id=$id";
    $query=mysqli_query($conexion,$sql);
    return $query;
}

function editarfechaA($conexion,$id,$fecha_apro){
    $sql="UPDATE subplaneacion_de_la_normatividad SET fecha_apro='$fecha_apro' WHERE id=$id";
    $query=mysqli_query($conexion,$sql);
    return $query;
}

function editarfechaIm($conexion,$id,$fecha_imp){
    $sql="UPDATE subplaneacion_de_la_normatividad SET fecha_imp='$fecha_imp' WHERE id=$id";
    $query=mysqli_query($conexion,$sql);
    return $query;
}

function editarUbicacion($conexion,$id,$ubicacion){
    $sql="UPDATE subplaneacion_de_la_normatividad SET ubicacion='$ubicacion' WHERE id=$id";
    $query=mysqli_query($conexion,$sql);
    return $query;
}
